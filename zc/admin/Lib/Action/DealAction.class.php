...
class DealAction extends CommonAction{
	...
	public function insert() {
		...
		if (false !== $list) {
			//成功提示
			$sql = "update fanwe_deal set price=".$_POST['price']." where id=".$list;
			$model = new Model();
			$model->execute($sql);
	...
	public function update() {
		...
		$shouyi = $_POST['shouyi'];
		$price = $_POST['price'];
		$focus_edit = $_POST['focus_edit'];
		$dolike_edit = $_POST['dolike_edit'];
		$share = $_POST['share'];

		$sql = "update fanwe_deal set shouyi='".$shouyi."' , price=".$price.",focus_edit=".$focus_edit.",dolike_edit=".$dolike_edit.",share=".$share." where id=".$data['id'];
		$db = new Model();
		$db->execute($sql);
		if (false !== $list) {
			if($data['is_effect']==1&&$data['user_id']>0)
	...
	public function index_image(){
		...
		$upload->savePath =  './public/attachment/index/';// 设置附件上传目录
		if(!$upload->upload()) {// 上传错误提示错误信息
			echo "<script>window.parent.uploadIndex(1,1)</script>";
		}else{// 上传成功
			$info =  $upload->getUploadFileInfo();
			$url = $info[0]['savepath'].$info[0]['savename'];
			
			$model = new Model();
			$sql = "update fanwe_deal set index_img='".$url."' where id=".$id;
			$result = $model->execute($sql);

			echo "<script>window.parent.uploadIndex(0,'".$url."')</script>";
		}	
	}
	
	public function msgadd(){
		$deal_id = $_GET['id'];
		$this->assign('id',$deal_id);
		$this->display();
	}
	...
	public function msginsert(){
		...
		$data['x'] = stripslashes($x);
		$data['deal_id'] = $deal_id;
		$data['addtime'] = $addtime;
		
		$result = M('dealmsg')->add($data);
		if($result){
			$this->success('添加成功');
		}
		else{
	...
	public function msglist(){
		...
		$list = M('dealmsg')->where('deal_id='.$deal_id)->select();
	...
	public function msgshow(){
		...
		$show = M('dealmsg')->where('deal_id='.$deal_id)->find();
		if ($show != null) {
			$res = M('dealmsg')->where('id='.$show['id'])->save('isshow=0');
		}
		$sql = "update fanwe_dealmsg set isshow=1 where id=".$id;
		//$result = M('dealmsg')->where('id='.$id)->data('isshow=1')->save();
		$model = new Model();
		$result = $model->execute($sql);
		if($result){
			$this->success('添加成功');
	...
	public function addkeyword(){
		...
		//dump($key);
		//exit;
		$sql = "update fanwe_deal_keyword set keyword='".$key."'";
		//echo $sql;
		//exit;
		$db = new Model();
	 ...
	 * keyword管理
	 */
	public function mangkeyword(){
		
		$key_list = M('deal_keyword')->find();
		$this->assign("key_list",$key_list);
		//dump($key_list) ;
		//exit;
		$this->display ('mangkeyword');
	}
