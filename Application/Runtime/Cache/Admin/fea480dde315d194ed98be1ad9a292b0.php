		...
		<div class="row">
			...
			<div class="span9">
				...
				<div class="widget widget-table">
					...
					<a href="javascript:;" onclick="$('#flash_pic').click()" class="btn btn-small btn-warning" style="margin-top:10px;margin-bottom:10px;">
						添加企业					
					</a>
						...
						<table class="table table-striped table-bordered">
							...
							<tbody>
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr style="height:100px">
										...
										<td><img alt="首页轮播图" style="height:80px;" src="<?php echo ($vo["img"]); ?>"></td>
										<td>
											<form action="<?php echo U('Pic/url');?>" method="post">
												<input type="text" style="width:90%" class="input-medium" name="url" value="<?php echo ($vo["url"]); ?>" onblur="$('#url_sub').click()">
												<input type="hidden" value="<?php echo ($vo["id"]); ?>" name="id">
												<input type="submit" id="url_sub" value="提交" style="display:none">
											</form>
										</td>
										<td class="action-td">
															
											<a href="<?php echo U('Pic/delFlash',array('id'=>$vo['id']));?>" class="btn btn-small">
